...
export const bankCardSchema = z.object({
  ...
  userName: z
    .string()
    .min(1, { message: "bankCard.validNameRequired" })
    // .min(1, { message: "Name is required" })
    .min(2, { message: "bankCard.validNameMin" })
    // .min(2, { message: "Name must be at least 2 characters" })
    .regex(/^[a-zA-Z\s]+$/, "bankCard.validNameLatin"),
  // .regex(/^[a-zA-Z\s]+$/, "String must contain only Latin letters"),
  month: z.string().min(1),
  year: z.string().min(4),
  cvc: z
    .string()
    .regex(/^\d+$/, { message: "bankCard.validCVCDigitsOnly" })
    // .regex(/^\d+$/, { message: "cvc must contain only digits" })
    .length(3, "bankCard.validCVCMin"),
